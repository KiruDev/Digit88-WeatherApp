...
open class AutoCompleteField: UITextField, UITableViewDataSource, UITableViewDelegate {
    ...
    fileprivate static let cellIdentifier = "D88SearchTextFieldCell"
    fileprivate let indicator = UIActivityIndicatorView(style:UIActivityIndicatorView.Style.large)
    ...
    override open func willMove(toSuperview newSuperview: UIView?) {
        ...
        self.addTarget(self, action: #selector(AutoCompleteField.textFieldDidChange), for: .editingChanged)
        self.addTarget(self, action: #selector(AutoCompleteField.textFieldDidBeginEditing), for: .editingDidBegin)
        self.addTarget(self, action: #selector(AutoCompleteField.textFieldDidEndEditing), for: .editingDidEnd)
        self.addTarget(self, action: #selector(AutoCompleteField.textFieldDidEndEditingOnExit), for: .editingDidEndOnExit)
        
        NotificationCenter.default.addObserver(self, selector: #selector(AutoCompleteField.keyboardWillShow(_:)), name: UIResponder.keyboardWillShowNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(AutoCompleteField.keyboardWillHide(_:)), name: UIResponder.keyboardWillHideNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(AutoCompleteField.keyboardDidChangeFrame(_:)), name: UIResponder.keyboardDidChangeFrameNotification, object: nil)
    }
    ...
    @objc open func textFieldDidChange() {
        ...
        timer?.invalidate()
        timer = Timer.scheduledTimer(timeInterval: typingStoppedDelay, target: self, selector: #selector(AutoCompleteField.typingDidStop), userInfo: self, repeats: false)
...
extension AutoCompleteField {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var cell = tableView.dequeueReusableCell(withIdentifier: AutoCompleteField.cellIdentifier)
        
        if cell == nil {
            cell = UITableViewCell(style: .subtitle, reuseIdentifier: AutoCompleteField.cellIdentifier)
        }
